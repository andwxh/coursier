...
final case class Resolution(
  ...
  def withDependencyManagement(project: Project): Project = {
    ...
    val project0 = withFinalProperties(
      project.copy(
        properties = parentProperties0 ++ project.properties
      )
    )
    ...
    val dependencies0 = addDependencies(
      Seq(project0.dependencies).map(withProperties(_, propertiesMap0))
    )
    val dependenciesMgmt0 = addDependencies(
      Seq(project0.dependencyManagement ).map(withProperties(_, propertiesMap0))
    )
    ...
    val depMgmt = (
      project0.dependencyManagement +: projs.map(_.dependencyManagement)
    )
